...
void SpellFunctions::init(lua_State* L) {
	...
	Lua::registerMethod(L, "Spell", "baseDamage", SpellFunctions::luaSpellBaseDamage);
	Lua::registerMethod(L, "Spell", "attackType", SpellFunctions::luaSpellAttackType);
	Lua::registerMethod(L, "Spell", "canCrit", SpellFunctions::luaSpellCanCrit);
	Lua::registerMethod(L, "Spell", "scaleDefense", SpellFunctions::luaSpellScaleDefense);
	Lua::registerMethod(L, "Spell", "ignoreArmor", SpellFunctions::luaSpellIgnoreArmor);
}
...
int SpellFunctions::luaSpellBaseDamage(lua_State* L) {
	const auto& spell = Lua::getUserdataShared<Spell>(L, 1, "Spell");
	if (!spell) {
		lua_pushnil(L);
		return 1;
	...
	if (lua_gettop(L) == 1) {
		lua_pushinteger(L, spell->getBaseDamage());
	} else {
		spell->setBaseDamage(luaL_checkinteger(L, 2));
		lua_pushboolean(L, true);
	}
	return 1;
}

int SpellFunctions::luaSpellAttackType(lua_State* L) {
	const auto& spell = Lua::getUserdataShared<Spell>(L, 1, "Spell");
	if (!spell) {
		lua_pushnil(L);
		return 1;
	...
	if (lua_gettop(L) == 1) {
		lua_pushinteger(L, static_cast<int>(spell->getAttackType()));
	} else {
		spell->setAttackType(static_cast<AttackType_t>(luaL_checkinteger(L, 2)));
		lua_pushboolean(L, true);
	}
	return 1;
}

int SpellFunctions::luaSpellCanCrit(lua_State* L) {
	const auto& spell = Lua::getUserdataShared<Spell>(L, 1, "Spell");
	if (!spell) {
		lua_pushnil(L);
		return 1;
	...
	if (lua_gettop(L) == 1) {
		lua_pushboolean(L, spell->getCanCrit());
	} else {
		spell->setCanCrit(lua_toboolean(L, 2));
		lua_pushboolean(L, true);
	}
	return 1;
}

int SpellFunctions::luaSpellScaleDefense(lua_State* L) {
	const auto& spell = Lua::getUserdataShared<Spell>(L, 1, "Spell");
	if (!spell) {
		lua_pushnil(L);
		return 1;
	...
	if (lua_gettop(L) == 1) {
		lua_pushboolean(L, spell->getScaleDefense());
	} else {
		spell->setScaleDefense(lua_toboolean(L, 2));
		lua_pushboolean(L, true);
	}
	return 1;
}

int SpellFunctions::luaSpellIgnoreArmor(lua_State* L) {
	const auto& spell = Lua::getUserdataShared<Spell>(L, 1, "Spell");
	if (!spell) {
		lua_pushnil(L);
		return 1;
	...
	if (lua_gettop(L) == 1) {
		lua_pushboolean(L, spell->getIgnoreArmor());
	} else {
		spell->setIgnoreArmor(lua_toboolean(L, 2));
		lua_pushboolean(L, true);
	}
	return 1;
}
...
/* 
int SpellFunctions::luaSpellCombatType(lua_State* L) {
	const auto& spell = Lua::getUserdataShared<Spell>(L, 1, "Spell");
	if (!spell) {
		lua_pushnil(L);
		return 1;
	...
	if (lua_gettop(L) == 1) {
		lua_pushinteger(L, static_cast<int>(spell->getCombatType()));
	} else {
		spell->setCombatType(static_cast<CombatType_t>(luaL_checkinteger(L, 2)));
		lua_pushboolean(L, true);
	}
	return 1;
} */
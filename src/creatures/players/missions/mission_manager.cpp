...
std::string MissionManager::getStageRequirementsStatus(Player& player, uint32_t missionId, uint32_t stageId) const {
    ...
    if (const auto skillAttr = stage.requirements.find("skill_name"); skillAttr != stage.requirements.end()) {
		...
		if (skillLevelAttr != stage.requirements.end()) {
			...
			g_logger().info("[CHECK SKILL] Skill Name: {}, Skill ID: {}, Player Level: {}, Required: {}",
				skillAttr->second, static_cast<uint32_t>(skillId), playerSkillLevel, requiredSkillLevel);
...
void MissionManager::onUseItem(Player& player, uint16_t itemId) const {
    ...
    for (const auto& objective : stage->objectives) {
        if ((objective.type == MissionObjectiveType::SABOTAGE || objective.type == MissionObjectiveType::INVESTIGATE) &&
            objective.param == itemKey) {
            ...
            player.sendTextMessage(MESSAGE_EVENT_ADVANCE,
                fmt::format("Updated objective: Used item {}!", itemKey));
...
void MissionManager::onCollectItem(Player& player, uint16_t itemId, uint32_t amount) const {
    ...
    for (const auto& objective : stage->objectives) {
        if (objective.type == MissionObjectiveType::COLLECT && objective.param == itemKey) {
            ...
            player.sendTextMessage(MESSAGE_EVENT_ADVANCE,
                fmt::format("Updated collect objective: {} ({}/{})", itemKey, newValue, objective.amount));
...
void MissionManager::onUseOnTarget(Player& player, uint16_t itemId, const std::string& targetName) const {
    ...
    for (const auto& objective : stage->objectives) {
        if (objective.type == MissionObjectiveType::USE_ON &&
            asLowerCaseString(objective.param) == targetKey &&
            ...
            player.sendTextMessage(MESSAGE_EVENT_ADVANCE,
                fmt::format("Updated USE_ON objective: {} ({}/{})", objective.param, newValue, objective.amount));
...
void MissionManager::completeTalkObjective(Player& player, const std::string& npcName) const {
    ...
    for (const auto& objective : stage->objectives) {
        if (objective.type == MissionObjectiveType::TALK &&
            asLowerCaseString(objective.param) == key) {
            ...
            player.sendTextMessage(MESSAGE_EVENT_ADVANCE,
                fmt::format("You talked to {} for your mission.", npcName));
        ...
        switch (objective.type) {
            ...
            case MissionObjectiveType::COLLECT:
            {
                ...
                if (removeCollectItems) {
                    ...
                    player.sendTextMessage(MESSAGE_EVENT_ADVANCE,
                        fmt::format("You delivered {}x of item {} for the mission.", required, itemId));
                }
                break;
            }
        ...
        switch (reward.type) {
            ...
            case MissionRewardType::SPELL: {
                if (!reward.name.empty()) {
                    ...
                    player.sendTextMessage(MESSAGE_EVENT_ADVANCE,
                        fmt::format("You have learned a new spell: {}!", reward.name));
                }
                break;
            }
